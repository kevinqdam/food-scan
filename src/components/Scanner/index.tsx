...
const drawGreenBoxesAroundCodeLikes = (
  data: QuaggaData,
  ctx: CanvasRenderingContext2D
) => {
  data
    .boxes!.filter(function (box: number[][]) {
      return box !== data.box;
    })
...
const drawRedLineThroughCode = (
  data: QuaggaData,
  ctx: CanvasRenderingContext2D
) => {
  Quagga.ImageDebug.drawPath(data.line, { x: 'x', y: 'y' }, ctx, {
    color: 'red',
    lineWidth: 3,
  });
};
...
const Scanner = function (props: ScannerProps) {
  ...
  useEffect(() => {
    Quagga.init(scannerConfig, function (err: string) {
      if (err) {
  ...
  return <div id='interactive' className='z-0 viewport' />;
};
